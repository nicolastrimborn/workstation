...
class CreateThread(Thread):
	...
	def run(self):
		if self.Thread_Name=="ActionsHandler":
			self.ActionsHandler()
		elif self.Thread_Name=="RequestsPoster":
			self.RequestsPoster()
	...
	def RequestsPoster(self):
		...
		while True:
			...
			#Post request
			try:
				r = requests.post("http://192.168.0.11:5000/conveyor/state", json={"state":State, "workstation":Workstation, "time":time.time()})
			except:
				print("Server not connected at 192.168.0.11, posting thread aborted, the workstation runs standalone !")
				break
